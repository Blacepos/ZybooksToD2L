"""
This file contains all of the code that I suspect may require changing from
semester to semester. For example: The name of the column D2L expects in the
output csv.
"""
...
# The exact format we expect the Zybooks csv to be in (truncating everything
# after "Percent Grade")
...
def get_activity_from_filename(filename: str) -> str:
    ...
    if result is not None:
        ...
        # It technically isn't possible for it to not be `str`, but I'm being
        # extra careful because I have trust issues with dynamic typing
        if isinstance(activity, str):
            return activity
        
        raise ZybooksFilenameUnexpectedMatch(
            "`result.group()` returned an \
unexpected type: `{}`. Value: `{}`".format(type(activity), activity)
        )
    
    raise ZybooksFilenameInvalidFormat(
        f"Expected csv filename to contain the string \"ActivityXX\" to \
identify the activity name. Got this instead: \"{filename}\""
    )
...
def zybooks_to_d2l(row: ZybooksRow) -> D2LRow:
    ...
    primary = row[PRIMARY_EMAIL_IDX]
    school = row[SCHOOL_EMAIL_IDX]
    grade = row[PERCENT_GRADE_IDX]
    ...
    try:
        ...
        raise ZybooksRowInvalidType(
            "Unable to cast type for a column in Zybooks row"
        )
    
    # make sure there's at least 1 MSU email
    if not util.is_msu_email(school):
        if not util.is_msu_email(primary):
            raise ZybooksRowNoMsuEmail(
                "Student does not appear to have an MSU email"
            )
    ...
    if grade > 100 or grade < 0:
        ...
        raise ZybooksRowPercentGradeNotInRange(
            f"Student grade column is not in range [0,100]. Got: {grade}"
        )
